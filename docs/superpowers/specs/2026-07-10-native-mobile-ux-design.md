...
- **한 번에 하나만 열림** — `ItemRow`는 `ItemList`(목록 화면)와 `CalendarView`(캘린더 상세) 양쪽에서 쓰인다. 열린 행 id를 부모 state로 들면 두 부모에 같은 로직을 중복해야 하므로, 저장하지 않는 작은 zustand 스토어 `src/lib/uiStore.ts`에 `openRowId`를 둔다. `ItemRow`가 직접 구독한다(`isOpen = openRowId === item.id`). 프롭 드릴링이 없고 `ItemList`는 서버 컴포넌트로 남는다.
...
- 배치: `layout.tsx`에 전역으로 둔다. 삭제는 목록 화면뿐 아니라 캘린더 상세(`CalendarView`)에서도 일어나므로 `ListPage`에만 두면 캘린더에서 삭제할 때 토스트가 뜨지 않는다.
- `position: fixed`로 하단에 띄우고, `bottom`은 `calc(env(safe-area-inset-bottom) + 5rem)`로 QuickAdd·탭바 위에 앉힌다. `z-50`.
...
- `src/app/layout.tsx` — `<ViewTransition>`, `<TabBar />`, `<UndoToast />`
...
`src/components/ItemList.tsx`와 `src/components/ListPage.tsx`는 변경하지 않는다.
